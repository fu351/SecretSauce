...
STORES = ["Target", "Kroger", "Meijer", "99Ranch", "Walmart", "Aldi", "Safeway", "TraderJoes"]
DEFAULT_ZIP_CODE = "94704"

# Path to Node.js scrapers (in lib/scrapers from project root)
NODE_SCRAPERS_PATH = Path(__file__).parent.parent / "lib" / "scrapers"

# Stop words for ingredient simplification
INGREDIENT_STOP_WORDS = {
    "fresh", "large", "small", "boneless", "skinless", "ripe", "optional",
    "chopped", "sliced", "diced", "minced", "ground", "crushed", "grated",
    "shredded", "cooked", "uncooked", "raw", "whole", "dried", "toasted",
    "packed", "divided"
}

def simplify_ingredient_name(value: str) -> str:
    """Remove preparation words and modifiers from ingredient name."""
    import re
    cleaned = re.sub(r'\(.*?\)', ' ', value)  # Remove parentheses
    cleaned = re.sub(r'[^a-z0-9\s]', ' ', cleaned.lower())  # Remove special chars
    tokens = [t for t in cleaned.split() if t and t not in INGREDIENT_STOP_WORDS]
    return ' '.join(tokens).strip()

async def scrape_ingredient_from_store(
    ingredient_id: str,
    ingredient_name: str,
    store: str,
    ...
    """
    Scrape a single ingredient from a specific store using Node.js scrapers.
    Returns the cheapest product found, or None if no results.
    """
    try:
        # Map store names to scraper filenames
        scraper_map = {
            "Target": "target.js",
            "Kroger": "kroger.js",
            "Meijer": "meijer.js",
            ...
            "Aldi": "aldi.js",
            "Safeway": "safeway.js",
            "TraderJoes": "traderjoes.js"
        }

        scraper_file = scraper_map.get(store)
        if not scraper_file:
            logger.warning(f"Unknown store: {store}")
            return None

        scraper_path = NODE_SCRAPERS_PATH / scraper_file
        if not scraper_path.exists():
            logger.error(f"Scraper not found: {scraper_path}")
            return None

        logger.info(f"[{store}] Scraping '{ingredient_name}'...")

        # Run Node.js scraper with original ingredient name
        result = await asyncio.to_thread(
            subprocess.run,
            ["node", str(scraper_path), ingredient_name, zip_code],
            capture_output=True,
            text=True,
            timeout=30
        )
        ...
        if result.returncode != 0:
            logger.warning(f"[{store}] Scraper failed for '{ingredient_name}': {result.stderr}")
            return None

        try:
            items = json.loads(result.stdout)
        except json.JSONDecodeError:
            logger.error(f"[{store}] Invalid JSON from scraper for '{ingredient_name}'")
            return None

        # If no results, try simplified name as fallback
        if not items and ' ' in ingredient_name:
            simplified = simplify_ingredient_name(ingredient_name)
            if simplified and simplified != ingredient_name:
                logger.info(f"[{store}] Retrying with simplified name: '{simplified}'")
                result = await asyncio.to_thread(
                    subprocess.run,
                    ["node", str(scraper_path), simplified, zip_code],
                    capture_output=True,
                    text=True,
                    timeout=30
                )
        ...
        if not items or len(items) == 0:
            logger.info(f"[{store}] No results for '{ingredient_name}'")
            return None

        # Find the cheapest item
        cheapest = min(items, key=lambda x: float(x.get('price', 999999)))

        return {
            "ingredient_id": ingredient_id,
            "store": store,
            "product_name": cheapest.get("title") or cheapest.get("name"),
            "price": float(cheapest.get("price", 0)),
            "quantity": cheapest.get("quantity", 1),
            "unit": cheapest.get("unit", "item"),
            "unit_price": float(cheapest["pricePerUnit"].replace('$', '')) if cheapest.get("pricePerUnit") else None,
            "image_url": cheapest.get("image_url"),
            "product_url": cheapest.get("product_url"),
            "product_id": str(cheapest.get("id")) if cheapest.get("id") else None
        }

    except subprocess.TimeoutExpired:
        logger.error(f"[{store}] Scraper timeout for '{ingredient_name}'")
        return None
    except Exception as e:
        logger.error(f"[{store}] Error scraping '{ingredient_name}': {e}")
        return None

@app.post("/daily-ingredient-scraper")
async def daily_ingredient_scraper(authorization: Optional[str] = Header(None)):
    """
    Scrape all standardized ingredients from all stores and cache results.
    ...
    logger.info("Starting daily ingredient scraper...")

    # Fetch all standardized ingredients
    try:
        response = supabase.table("standardized_ingredients").select("id, canonical_name, category").execute()
    ...
    if not ingredients:
        return {"success": True, "message": "No ingredients to scrape", "cached": 0, "failed": 0}

    logger.info(f"Found {len(ingredients)} ingredients to scrape")
    ...
    for ingredient in ingredients:
        ...
        # Scrape all stores for this ingredient in parallel
        tasks = [
            scrape_ingredient_from_store(ingredient_id, canonical_name, store)
            for store in STORES
        ]
        ...
        for store_result in store_results:
            if isinstance(store_result, Exception):
                results["failed"] += 1
                results["errors"].append(str(store_result))
                continue
            ...
            # Insert/update cache in Supabase
            try:
                cache_data = {
                    "standardized_ingredient_id": store_result["ingredient_id"],
                    "store": store_result["store"],
                    "product_name": store_result["product_name"],
                    "price": store_result["price"],
                    "quantity": store_result["quantity"],
                    "unit": store_result["unit"],
                    "unit_price": store_result["unit_price"],
                    "image_url": store_result["image_url"],
                    "product_url": store_result["product_url"],
                    "product_id": store_result["product_id"],
                    "expires_at": expires_at,
                    "updated_at": datetime.utcnow().isoformat()
                }

                supabase.table("ingredient_cache").upsert(
                    cache_data,
                    on_conflict="standardized_ingredient_id,store"
                ).execute()

                results["cached"] += 1
                logger.info(f"Cached {canonical_name} from {store_result['store']}: ${store_result['price']}")

            except Exception as e:
                logger.error(f"Failed to cache {canonical_name} from {store_result['store']}: {e}")
                results["failed"] += 1
                results["errors"].append(f"{canonical_name} @ {store_result['store']}: {str(e)}")

    logger.info(f"Daily scraper completed: {results['cached']} cached, {results['failed']} failed")
