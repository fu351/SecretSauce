 ...
 *
 * Usage:
 *   node scripts/test-target-fixed.js
 */

require('dotenv').config();
const { getTargetProducts, getNearestStore } = require('../lib/scrapers/target.js');
...
async function testTargetScraper() {
    ...
    try {
        ...
        if (!store) {
            ...
            if (store.distance_miles !== undefined) {
                console.log(`   Distance: ${store.distance_miles.toFixed(2)} miles`);
            }
            console.log('');
