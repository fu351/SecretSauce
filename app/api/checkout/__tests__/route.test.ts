...
describe("POST /api/checkout", () => {
  ...
  it("creates a Stripe customer, updates profile, and applies discount for non-active subscribers", async () => {
    ...
    expect(sessionConfig.metadata.cart_items).toContain('"product_id":"p1"')
  })
  ...
  it("falls back to email identity, links clerk id, and excludes oversized cart metadata", async () => {
    ...
    const largeCart = [
      {
        item_id: "x".repeat(250),
        product_id: "y".repeat(250),
        num_pkgs: 1,
        frontend_price: 9.99,
      },
    ]
    const res = await POST(makeRequest({ cartItems: largeCart }))
