...
import { assertSocialEnabled, remixMealPlanShare } from "@/lib/social/service"
...
export async function POST(req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const result = await remixMealPlanShare(profile.supabase as any, {
    profileId: profile.profileId,
    shareId: routeParams.id,
    targetWeekIndex: typeof body.targetWeekIndex === "number" ? body.targetWeekIndex : null,
    idempotencyKey: typeof body.idempotencyKey === "string" ? body.idempotencyKey : null,
  })
  if ("validationError" in result) return NextResponse.json({ error: result.validationError }, { status: 409 })
  if ("error" in result && result.error) return NextResponse.json({ error: "Failed to remix meal plan" }, { status: 500 })
