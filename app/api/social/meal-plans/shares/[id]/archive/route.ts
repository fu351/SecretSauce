...
import { assertSocialEnabled, archiveMealPlanShare } from "@/lib/social/service"
...
export async function POST(_req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const result = await archiveMealPlanShare(profile.supabase as any, {
    profileId: profile.profileId,
    shareId: routeParams.id,
  })
  if ("validationError" in result) return NextResponse.json({ error: result.validationError }, { status: 409 })
  if ("error" in result && result.error) return NextResponse.json({ error: "Failed to archive meal plan share" }, { status: 500 })
