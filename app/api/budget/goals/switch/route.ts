...
import { assertBudgetEnabled, switchBudgetGoal } from "@/lib/budget/service"
  ...
  try {
    ...
    const idempotencyKey =
      typeof body.idempotencyKey === "string" && body.idempotencyKey.trim().length > 0
        ...
        : buildIdempotencyKey(["budget-switch", profile.profileId, name, targetCents, category, Date.now()])
    ...
    const result = await switchBudgetGoal(profile.supabase as any, {
    ...
    if ("error" in result && result.error) {
      console.error("[budget/goals/switch POST] DB error:", result.error)
    ...
    return NextResponse.json({ goal: result.goal, previousGoalId: result.previousGoalId })
  } catch (error) {
    console.error("[budget/goals/switch POST] Unexpected error:", error)
