...
async function getDashboardData(filters: ModerationFilters) {
  ...
  const filteredFlags = openFlags
    ...
    .filter((flag) =>
      matchesSearch(flag.recipe_title, flag.recipe_id, flag.reason, flag.details, flag.reporter_name, flag.reporter_username)
    )
    .sort((a, b) => severityRank(b.severity) - severityRank(a.severity))

  const filteredRecentRecipes = ((recentRecipes || []) as RecipeRow[]).filter((recipe) => {
    if (filters.flaggedOnly && !flagCounts[recipe.id]) return false
    return matchesSearch(recipe.title, recipe.id, recipe.author_id)
  })
  ...
  return {
    ...
    filteredFlags,
    highOpenFlags,
    recentRecipes: filteredRecentRecipes,
    flagCounts,
  }
}
...
async function deleteRecipeAndResolveFlagAction(formData: FormData) {
  "use server"
  const admin = await getAdminUser()

  const recipeId = String(formData.get("recipe_id") ?? "")
  const flagId = String(formData.get("flag_id") ?? "")
  if (!recipeId || !flagId) return

  const supabase = createServiceSupabaseClient()
  await supabase.from("recipes").update({ deleted_at: new Date().toISOString() }).eq("id", recipeId)

  const flagsDB = new RecipeFlagsDB(supabase)
  await flagsDB.resolveFlag(flagId, admin.id, "Recipe removed from circulation via moderation queue", "resolved")
                    ...
                    <tr key={flag.id} className="align-top hover:bg-gray-50">
                      <Td className="min-w-64 whitespace-normal">
                        <div className="space-y-1">
                          ...
                          <div className="flex flex-wrap gap-2 pt-1">
                            <Link href={`/recipes/${flag.recipe_id}`} className="text-xs font-medium text-blue-600 hover:text-blue-700">
                              View recipe
                            </Link>
                            <Link href={`/edit-recipe/${flag.recipe_id}`} className="text-xs font-medium text-blue-600 hover:text-blue-700">
                              Edit recipe
                            </Link>
                          </div>
                        </div>
                      ...
                      <Td>
                        <div className="flex flex-wrap gap-2">
                          ...
                          <form action={deleteRecipeAndResolveFlagAction}>
                            <input type="hidden" name="recipe_id" value={flag.recipe_id} />
                            <input type="hidden" name="flag_id" value={flag.id} />
          ...
          <div className="mb-4 flex items-center justify-between gap-3">
            <div>
              ...
              <p className="text-sm text-gray-600">
                Showing {recentRecipes.length} recent recipes{filters.flaggedOnly ? " with open flags" : ""}.
              </p>
            </div>
          </div>
...
function ModerationFiltersForm({ filters }: { filters: ModerationFilters }) {
  return (
    <form className="flex flex-wrap items-end gap-2" action="/dev/recipes">
      <div className="space-y-1">
      ...
      <Button asChild size="sm" variant="ghost">
        <Link href="/dev/recipes">Reset</Link>
      </Button>
    </form>
  )
}
