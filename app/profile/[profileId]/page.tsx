...
export default async function ProfilePage({ params }: Props) {
  ...
  const { followerCount, followingCount } = await db.getCounts(profileId)
  ...
  if (authState.userId) {
    ...
    if (viewerProfile) {
      isOwnProfile = viewerProfile.id === profileId

      if (!isOwnProfile) {
        const result = await db.getFollowStatus(viewerProfile.id, profileId)
