...
async function resolveBatch(
  ...
  const cachedEmbeddings = await embeddingQueueDB.fetchCandidateEmbeddingsByInputTexts(
    inputTexts,
    model
  )

  // Build a positional map from original row index → miss position so the
  // reconstruction step below is O(n) instead of O(n²).
  ...
  const rowIndexToMissPos = new Map(missIndices.map((origIdx, missPos) => [origIdx, missPos]))
  ...
  const embeddings: number[][] = rows.map((row, i) => {
    ...
    return ollamaVectors[rowIndexToMissPos.get(i)!]
  })
...
export async function runEmbeddingQueueResolver(
  config: EmbeddingWorkerConfig
  ...
  while (true) {
    ...
    if (!config.dryRun) {
      const requeued = await embeddingQueueDB.requeueExpired(
        config.requeueLimit,
      ...
      if (requeued > 0) {
        console.log(
          `[EmbeddingQueueResolver] ${modePrefix}Requeued ${requeued} expired processing row(s)`
        )
      }
    }
    ...
    if (!rows.length) {
      if (cycle === 0) {
        ...
        console.log(
          `[EmbeddingQueueResolver] ${modePrefix}Queue drained after ${cycle} cycle(s)`
        )
      }
      break
    }
    ...
    try {
      ...
      if (!config.dryRun) {
        await Promise.allSettled(
          rows.map((row) => embeddingQueueDB.markFailed(row.id, message))
        )
      }
    }
