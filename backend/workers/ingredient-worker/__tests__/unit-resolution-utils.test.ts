...
describe('shouldUsePackagedUnitFallbackAfterFailure', () => {
  ...
  it('returns true when scraper and no unit signals and unit failed', () => {
    const row = makeRow({ source: 'scraper', cleaned_name: 'olive oil', raw_product_name: 'kirkland olive oil' })
    expect(shouldUsePackagedUnitFallbackAfterFailure(row, { id: '1', status: 'error', resolvedUnit: null, resolvedQuantity: null, confidence: 0 })).toBe(true)
  })
...
describe('isPackagedUnitFallbackResult', () => {
  ...
  it('returns false when status is not success', () => {
    expect(isPackagedUnitFallbackResult(row, { id: '1', status: 'error', resolvedUnit: null, resolvedQuantity: null, confidence: 0 })).toBe(false)
  })
...
describe('stripMeasurementFromSearchTerm', () => {
  ...
  it('strips leading quantity and unit', () => {
    const unitResult = { id: '1', status: 'success' as const, resolvedUnit: 'oz' as const, resolvedQuantity: 2, confidence: 0.9 }
    const result = stripMeasurementFromSearchTerm('2 oz flour', row, unitResult)
  ...
  it('handles fractional quantities', () => {
    const unitResult = { id: '1', status: 'success' as const, resolvedUnit: 'oz' as const, resolvedQuantity: 0.5, confidence: 0.9 }
    const result = stripMeasurementFromSearchTerm('1/2 oz sugar', row, unitResult)
