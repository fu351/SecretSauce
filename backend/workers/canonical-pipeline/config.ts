...
export interface CanonicalPipelineConfig {
  ...
  // Probation-embedding stage — forwarded to EmbeddingWorkerConfig (mode forced to "probation-embedding")
  probationBatchLimit: number
  probationMinDistinctSources: number
  ollamaBaseUrl: string
  embeddingModel: string

  // Vector discovery stage — forwarded to VectorDoubleCheckWorkerConfig
  vectorSimilarityThreshold: number
  vectorBatchLimit: number
  vectorEmbeddingModel: string

  // Consolidation stage — forwarded to CanonicalConsolidationWorkerConfig
