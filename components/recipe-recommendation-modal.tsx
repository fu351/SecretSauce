      ...
      if (user && favoriteIds.size > 0) {
        ...
        allRecipes.forEach((r) => {
          if (favoriteIds.has(r.id)) {
            ...
            if (r.tags) userLikedTags.add(r.tags.toLowerCase())
          }
        })
        ...
        if (recipe.tags && userLikedTags.has(recipe.tags.toLowerCase())) {
          score += 0.5
        }
      }
  ...
  const recommendations = useMemo(() => {
    ...
    if (searchQuery.trim()) {
      ...
      results = filtered.filter((recipe) => {
        ...
        const matchesTags = recipe.tags?.toLowerCase().includes(query)
