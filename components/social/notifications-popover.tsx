...
import type { Notification, ProfileSnippet } from "@/components/social/notifications-widget"
...
export function NotificationsPopover() {
  ...
  const profileHref = (p: ProfileSnippet) => p.username ? `/user/${p.username}` : "#"

  return (
    <Popover onOpenChange={(open) => { if (open) setSeenCount(notifications.length) }}>
      <PopoverTrigger asChild>
        <Button variant="ghost" size="icon" className="relative hover:bg-muted/60" aria-label="Notifications">
          ...
          {!loading && unseenCount > 0 && (
            <span className="absolute top-1 right-1 flex h-4 min-w-4 items-center justify-center rounded-full bg-red-500 px-0.5 text-[10px] font-semibold text-white leading-none">
              {badgeLabel}
            </span>
          )}
        </Button>
