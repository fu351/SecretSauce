...
- `workers/canonical-consolidation-worker/config.ts` - reads runtime config from env.
- `workers/canonical-consolidation-worker/processor.ts` - fetches candidates, applies guards, performs merges, and writes logs.
- `workers/canonical-consolidation-worker/guards.ts` - rejects risky candidates before merge.
- `workers/canonical-consolidation-worker/survivor.ts` - chooses the survivor vs loser canonical.
- `lib/database/canonical-consolidation-db.ts` - Supabase access for fetch/merge/log operations.
- `workers/canonical-consolidation-worker/__tests__/guards.test.ts` - guard coverage.
...
```bash
tsx --env-file=.env.local workers/canonical-consolidation-worker/resolve-canonical-consolidation.ts
```
...
```bash
vitest run workers/canonical-consolidation-worker/__tests__/guards.test.ts
```
