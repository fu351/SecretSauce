...
async function getAuthToken() {
    try {
        if (!CLIENT_ID || !CLIENT_SECRET) {
            console.error("[kroger] Missing Kroger API credentials");
            return null;
        }
        ...
        if (!token) {
            console.error("[kroger] Auth token response missing access_token");
            return null;
        }

        return token;
    } catch (error) {
        console.error("[kroger] Error fetching auth token:", error.response?.data || error.message);
        return null;
    }
}
...
async function getNearestStore(zipCode, authToken) {
    try {
        ...
        if (!Array.isArray(locations) || locations.length === 0) {
            console.warn("[kroger] No location found for ZIP:", zipCode);
            return null;
        }
        ...
        console.error("[kroger] Error fetching location ID:", error.response?.data || error.message);
        return null;
    }
}
...
async function getProducts(searchTerm, locationId, authToken, brand = '') {
    try {
        if (!locationId) {
            console.warn("[kroger] Missing locationId for product lookup");
            return [];
        }
        ...
        if (!Array.isArray(products) || products.length === 0) {
            console.warn("[kroger] No products found for search term:", searchTerm);
            return [];
        }
        ...
        console.error("[kroger] Error fetching products:", error.response?.data || error.message);
        return [];
    }
}
...
async function Krogers(zipCode = 47906, searchTerm, brand = '') {
    try {
        ...
        if (!searchTerm || !String(searchTerm).trim()) {
            console.warn("[kroger] Missing searchTerm");
            return [];
        }
        ...
        console.log("[kroger] Store resolved", {
        ...
        console.error("[kroger] Error in Krogers function:", error.message || error);
        return [];
    }
}
