...
async function searchWholeFoods(keyword, zipCode) {
    try {
        console.log(`Searching Whole Foods for: ${keyword} in zip: ${zipCode}`);
        
        // Whole Foods doesn't have a public API, so we'll return empty results
        console.log("Whole Foods doesn't have a public API, returning empty results");
        return [];

    } catch (error) {
        console.error("Error searching Whole Foods products:", error.message);
        return [];
    }
}
    ...
    try {
        ...
        if (scriptMatches) {
            try {
                ...
                console.error("Failed to parse Whole Foods JSON:", e.message);
            }
        }
        ...
        if (productMatches) {
            try {
                ...
                console.error("Failed to parse Whole Foods initial state:", e.message);
            }
        }
        
        return [];
    } catch (error) {
        console.error("Error extracting Whole Foods products from HTML:", error.message);
        return [];
    }
}
...
if (require.main === module) {
    ...
    if (!keyword || !zipCode) {
        console.error("Usage: node wholefoods.js <keyword> <zipCode>");
        process.exit(1);
    }
