...
async function getNearestStore(zipCode) {
    ...
    try {
        ...
        return {
            id: storeId,
            name: store?.store_name || store?.storeName || "Target",
            address: {
                line1: address?.line1 || address?.address_line_1 || address?.addressLine1 || "",
                city: address?.city || address?.city_name || "",
                state: address?.region || address?.state || address?.state_code || "",
                postalCode: address?.postal_code || address?.zip || address?.zipCode || zipCode,
            },
            raw: store,
        };
...
function formatTargetStoreLocation(storeInfo, fallbackZip) {
    if (!storeInfo) {
        return fallbackZip ? `Target (${fallbackZip})` : "Target Store";
    }
    ...
    return storeInfo.name || "Target Store";
}
