 ...
 */
async function getNearestStore(location, radiusMiles = 20) {
    ...
    try {
        ...
        // Case 2: ZIP code provided - find by ZIP with coordinates
        else if (typeof location === 'string') {
            const zipCode = location;
            targetDebug(`[target] Finding nearest store using ZIP code: ${zipCode}`);

            // Use raw SQL to get store with extracted lat/lng from PostGIS geometry
            try {
                const dbModule = await import('../database/supabase.js');
                ...
                if (error || !storeData) {
                    targetDebug(`[target] No store found in database for ZIP ${zipCode}, falling back to API`);
                    return await getNearestStoreFromTargetAPI(zipCode);
                }

                // Now get coordinates using RPC function
                const { data: coordData, error: coordError } = await supabase
                    .rpc('get_store_coordinates', { store_id: storeData.id });

                if (coordError || !coordData || !coordData.lat || !coordData.lng) {
                    targetDebug(`[target] Could not extract coordinates for store ${storeData.id}, falling back to API`);
                    return await getNearestStoreFromTargetAPI(zipCode);
                }
                ...
                targetDebug(`[target] Found store from database with coordinates: ${store.lat}, ${store.lng}`);

            } catch (error) {
                targetDebug(`[target] Error querying database for ZIP ${zipCode}: ${error.message}`);
                return await getNearestStoreFromTargetAPI(zipCode);
            }
        }
 ...
 */
async function getNearestStoreFromTargetAPI(zipCode) {
    ...
    const requestPromise = (async () => {
        try {
            ...
            if (response.status === 404) {
                log.warn(`[target] API returned 404 for "${keyword}" at store ${storeId} (${zipCode})`);
                targetDebug(`[target] Request URL: ${baseUrl}`);
                return [];
            }
            ...
            const errorStoreId = storeMetadata?.id || 'unknown';
