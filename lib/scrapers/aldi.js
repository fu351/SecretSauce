...
async function withRetry(fn, options = {}) {
    ...
    for (let attempt = 0; attempt <= maxRetries; attempt++) {
        try {
            ...
            console.log(`Attempt ${attempt + 1}/${maxRetries + 1} with timeout ${currentTimeout}ms`);
...
async function crawlAldiWithJina(keyword, zipCode) {
    try {
        ...
        const response = await withRetry(async (currentTimeout, attempt) => {
            console.log(`Jina AI request for Aldi (attempt ${attempt + 1})`);
        ...
        console.log(`Successfully retrieved content from Jina AI (${response.data.length} chars)`);

        // Jina returns the clean markdown text directly
        return response.data;

    } catch (error) {
        console.error("Error crawling with Jina AI after all retries:", error.message);
        return null;
    }
}
...
Return only the JSON array, no other text.`;
        ...
        const response = await withTimeout(
            ...
            Math.min(REQUEST_TIMEOUT_MS, 20000)  // Use configured timeout, capped at 20 seconds
        );
