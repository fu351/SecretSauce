...
async function Meijers(zipCode = 47906, searchTerm) {
    try {
        ...
        try {
            ...
            console.warn("Unable to resolve nearest Meijer location:", locationError.message || locationError);
        }
        ...
        if (!filteredProducts.length) {
            console.warn("All products filtered out for search term:", searchTerm);
            return [];
        }
...
function extractNearestStore(locationsResponse) {
    ...
    const address = store?.storeAddress || store?.address || store?.contact?.address || {};
    ...
    return {
        ...
        name: store?.storeName || store?.name || "Meijer",
        city: store?.city || address?.city || store?.storeCity,
        state: store?.state || address?.state || address?.stateAbbreviation,
        postalCode: store?.zipCode || address?.zipCode || address?.postalCode,
    };
}
...
function formatMeijerStoreLocation(storeInfo, fallbackZip) {
    ...
    return storeInfo?.name || "Meijer Store";
}
