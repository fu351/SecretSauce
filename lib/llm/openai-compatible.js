...
function resolveLlmModelForUrl(url, {
  defaultModel = "gemma3:4b",
  defaultOpenAiModel = "gpt-4o-mini",
} = {}) {
  const explicit = trim(process.env.LLM_MODEL)
  if (explicit) return explicit

  if (isOpenAiChatCompletionsUrl(url)) {
    return trim(process.env.OPENAI_MODEL) || defaultOpenAiModel
  }
...
async function requestChatCompletion({
  ...
  const resolvedModel = model || resolveLlmModelForUrl(resolvedUrl)
